...
function percentage(target, context) {
  ...
  if (target.slice(-1) !== context.slice(-1)) {
    throw new Error(
      "Cannot calculate percentage; units do not appear to match."
    );
  }
...
function span(columns, math = settings.math) {
  const error =
    "You must pass a number greater than 0 for the number of columns.";
  let spanColumns = 0;
  ...
  if (columns) {
    ...
    if (
      settings.gutterPosition === "inside" ||
...
export { container, gutter, span, add, subtract, multiply, divide, percentage };
